...
const GLOBAL = {
    ...
    getModel: function(obj, is, value) {
        try {
            if (typeof is == 'string') {
                ...
                if (value == '_delete_') return delete obj[is[0]];
                else {
    ...
    drawChart: function(chart_data, element) {
        const pie_data = GoogleCharts.api.visualization.arrayToDataTable(
            chart_data
        );
        const pie_options = {
            // pieHole: 0.6,
        ...
        const pie_chart = new GoogleCharts.api.visualization.PieChart(
