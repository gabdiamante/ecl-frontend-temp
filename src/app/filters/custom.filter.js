    ...
    angular
        ...
        function handlefetch ($sce) {
            return function (input, text, text_muted) {
                ...
                var att = '<em ng-class="{\'text-muted\': ' + text_muted + '}"><i class="'+ input_state +'"></i>' + input_text + '</em>';
		...
		function handleStatus ($sce) {
        	return function (input) { 
                var class_status = 
                    (input == 'APPROVED' || input == 'ACTIVE') ? 'success' : 
                    (input == 'PENDING') ? 'primary' : 
                    (input == 'BANNED') ? 'warning' : 
                    (input == 'DISAPPROVED' || input == 'DEACTIVATED') ? 'danger' : 'danger';
        ...
        function handleGlyphicon ($sce) {
            return function (input) {
                var input_state =   (input) ? 'glyphicon glyphicon-ok text-success' : 
                                    (!input) ? 'glyphicon glyphicon-remove text-danger' : 
        ...
        function handleSpan ($sce) {
            return function (input) {
                var input_state =   (input) ? 'label label-info' : 
                                    (!input) ? 'label label-success' : 
                                    'label label-danger'; 
                var input_text =    (input) ? 'Yes' : 
                                    (!input) ? 'No' : 'No';

                var att = '<span class="' + input_state + '">' + input_text + '</span>';
                return $sce.trustAsHtml(att);
            };
        }
        ...
        function displaynone ($sce) {
            return function (input , pre, sup) {
                 return $sce.trustAsHtml(
                        (input || input === 0) ? (pre?pre : '') + input + (sup?sup:''): "<span class='text-uppercase text-italic low-opacity'>None</span>"
                 );
            };
        }
            ...
            return function(items,objSearch, field_name){
                ...
                var recursiveFilter = function(items,objSearch, field_name){
                    ...
                    angular.forEach(items,function(item){
                        ...
                        if(angular.isArray(item[field_name]) && item[field_name].length > 0){
                            recursiveFilter(item[field_name],objSearch, field_name);              
                        }
                    });
                };
        ...
        function filterBranch($filter){
           ...
           return function(items,objSearch, field_name){
              var filtered = [];
              var return_f =[];
              var recursiveFilter = function(return_f, property_name, valueSearch){

                    var return_e= $filter('filterTree')(items,{[property_name] : return_f.parentId }, field_name)[0];
                    if (return_e) {
              ...
              if (valueSearch) {
                    return_f= $filter('filterTree')(items,{[property_name] : valueSearch }, field_name)[0];
                    if (!return_f) {
                        return_f= $filter('filterTree')(items,{[property_name] : parseInt(valueSearch) }, field_name)[0];
                    }
        ...
        function filterTreeAttributeFields($filter){
            ...
            return function(items, field_names, childrenProperty){
                ...
                var recursiveFilter = function(items, field_names, childrenProperty){
                    angular.forEach(items,function(item){
                        //console.log(item);
                        for (var f in field_names) {
                            var field_name_val = field_names[f];
                            item[f] = $filter('filterAttributes')(item,field_name_val).value;
                        }

                        recursiveFilter(item[childrenProperty],field_names, childrenProperty);              
                    });
                };
        ...
        function applyValue () {
            return function (input , pre, sup) { 
                if (pre == 'useInLayeredNavigation') {
                    var output =    (input === 0) ? 'No' : 
                                    (input === 1) ? 'Filterable (with results)' : 
                                    (input === 2) ? 'Filterable (no results)' : 'No';
                    return output;
                } else if (pre == 'isActive') {
                    var output =    (input === 0) ? 'Inactive' : 
        ...
        function filterAttributes($filter) {
            return function(item, value, propertyParentNameAttribute, propertyChildNameAttribute) {
                ...
                if (item) {
                    var filtered = $filter('filter')(item[propertyParentNameAttribute], { [propertyChildNameAttribute] : value}, true );
        ...
        function filterMultipleAttributes($filter) {
            return function(item, arrayValues, propertyParentNameAttribute, propertyChildNameAttribute) {
                var filtered_object = {};
                var attrName ='';

                for (var i=0; i<arrayValues.length; i++) {
                    attrName = arrayValues[i];
                    filtered_object[attrName] = $filter('filterAttributes')(item, attrName, propertyParentNameAttribute, propertyChildNameAttribute);
                }
        ...
        function hasTag() {
            return function(items, property_name , value) {
                ...
                angular.forEach(items, function(el) {
                    if(el[property_name] && (el[property_name].indexOf(value)>-1 || el[property_name].indexOf(value.toString())>-1 || el[property_name].indexOf(parseInt(value))>-1) ) {
                        filtered.push(el);
                    }
                });
                return filtered;
        ...
        function handleAttributesDisplay() {
            return function(data, attributeArrObjName) {
                ...
                for (var i = 0; i < data.length; i++) {
                    data[i][attributeArrValuesName] = data[i][attributeArrValuesName] || {};
                    data[i][attributeArrObjName] = data[i][attributeArrObjName] || [];
                    for (var a = 0; a < data[i][attributeArrObjName].length; a++) {
                        data[i][attributeArrValuesName][data[i][attributeArrObjName][a].attributeCode] = data[i][attributeArrObjName][a].value;
                    }
                }
                return data;
            }
        ...
        function handleAttributesDisplayParent() {
            return function(data, attributeArrObjName) {
                ...
                for (var i = 0; i < data.length; i++) {
                    data[i][attributeArrObjName] = data[i][attributeArrObjName] || [];
                    for (var a = 0; a < data[i][attributeArrObjName].length; a++) {
                        data[i][data[i][attributeArrObjName][a].attributeCode] = data[i][attributeArrObjName][a].value;
                    }
                }
                return data;
            };
        ...
        function limitHtml() {
            return function(text, limit, char) {
                ...
                var changedString = String(text).replace(/<[^>]+>/gm, '').replace(/\n/g, " ").replace(/\r/g, " ");
                var length = changedString.length;

                if (!char) char = '...';

                return changedString.length > limit ? (changedString.substr(0, limit - 1) + ' ' + char) : changedString; 
            };
        }
        ...
        function handleStores ($filter) {
            return function (value, data, key) {
                if (data)
                    return $filter('filter')(data, { groupId: value }, true)[0][key];
            };
        }
