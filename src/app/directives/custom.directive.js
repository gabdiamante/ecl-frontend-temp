         ...
         return {
            ...
            link: function($scope, $element, $attrs) {
                ...
                $scope.$watch(
                    ...
                    function(bool) {
                        if (bool) {
                            $element.attr($scope.dynamicAttr, $scope.dynamicValue);
                        }
                        $compile($element)($scope);
                    }
                );
        ...
        return {
            ...
            link: function (scope, element, attrs) {
                scope.$watch('ngIfCond', function (value) {
                    if (!value) {
                        for (let i=0; i< scope.scopeData.removeHideData.length; i++)
                            GLOBAL.getModel(scope.modelData, scope.scopeData.removeHideData[i], '_delete_');
    ...
    function complexTable () {
        return {
            ...
            controller: function($scope) {
                ...
                $scope.reset = (model) => {
                        GLOBAL.getModel($scope,model, angular.copy($scope.vm.search_old));
                };
            },
        };
    }
                ...
                element.bind('error', function () {
                    $timeout(function(){
                        ...
                        if (typeof DEFAULT_IMAGE == 'undefined')
                            url = 'http://placehold.it/' + w + 'x' + h + '/cccccc/ffffff&text=No Image!';
                        else 
                            url = DEFAULT_IMAGE;
                ...
                var attributes_values = $scope.urlAttributesValues;
                 ...
                 if (typeof $scope.urlBlobDetails == 'string') {
                    ...
                    Upload.urlToBlob(fileUrl).then(function(blobFile) {  
                        blobFile = new Blob([blobFile],{type:"image"}) || {};
                        Upload.imageDimensions(blobFile).then(function(dimensions){                          
